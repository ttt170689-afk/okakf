...
(function (global) {
  ...
  const CONFIG = {
    ...
    player: {
      ...
      eyeHeight: 1.62,
      crouchHeight: 0.95,
      crawlHeight: 0.52,          // ползком — пролезть под живот и мебель
      /* --- ДВУХСЛОЙНОЕ ТЕЛО ДРУГА ---
       * Игрок мягко утопает в жире на sinkBase + sinkSoft метров, но глубже
       * упирается во «внутренний слой» (мышцы) и внутрь туши не проходит.
       * Это одновременно убирает воздушную стенку и не даёт застрять внутри. */
      sinkBase: 0.10,      // минимальное продавливание даже твёрдой зоны, м
      sinkSoft: 0.45,      // надбавка за мягкость: живот пускает до ~55 см
      sinkBurrow: 0.14,    // бонус при карабканье/ползании по телу
      /* Анти-баг: если игрок не может выбраться из тела */
      stuckSeconds: 2.0,   // столько неподвижности внутри = режим «фантом»
      stuckFreeSeconds: 3.0, // столько же = авто-выталкивание наружу
      walkSpeed: 3.6,
      runSpeed: 6.4,
      climbSpeed: 1.5,
      jumpVelocity: 5.1,
      ...
      /* --- ПОГРУЖЕНИЕ РУК В ЖИР ПРИ ХВАТЕ ---
       * Глубина в МЕТРАХ (не масштабируется размером друга: продавливаемость
       * — свойство плоти). Считается динамически от мягкости и налитости
       * места, куда легла ладонь:
       *     depth = grabSinkMin + softness * growth * grabSinkSoft
       * Итог по ТЗ: живот/попа 35–45 см, бока/бёдра 25–35,
       * руки/шея 15–25, голова/лапки 5–15. */
      grabSinkMin: 0.05,    // самая твёрдая точка (лапка, бровь)
      grabSinkSoft: 0.42,   // надбавка за мягкую налитую плоть
      grabSinkMax: 0.45,    // жёсткий потолок: глубже — рука в мышцах
      grabBulge: 1.85,      // во сколько раз валик шире ямки
      pokeImpulse: 3.4,
      slapImpulse: 7.5,
      massageImpulse: 0.55,
    },
    ...
    growth: {
      ...
      baseMassKg: 62,
      /* Стадии 0..100. До 10-й — обычный путь, дальше «пост-игра»:
       * друг перестаёт быть существом привычного масштаба и дорастает
       * до планетарной формы (см. stage 100 «Сфера Бытия»). Пороги идут
       * геометрически, поэтому поздние стадии — долгая, но достижимая цель. */
      stageThresholds: (() => {
        const t = [0, 250, 1200, 4500, 12000, 28000, 60000, 110000, 220000, 400000, 800000];
        // 11..100: каждый шаг тяжелее предыдущего на ~11.5%
        let v = 800000;
      ...
      stageNames: (() => {
        ...
        const late = [
          'Титан', 'Исполин', 'Гора Плоти', 'Континент', 'Левиафан',
          'Бегемот', 'Колоссаль', 'Мегатонна', 'Гигатонна', 'Небесный Кит',
          'Лунный Друг', 'Планетоид', 'Малая Планета', 'Газовый Гигант', 'Звёздный Пух',
          'Сфера Бытия',
        ];
        for (let i = 11; i <= 100; i++) {
          const idx = Math.min(late.length - 1, Math.floor((i - 11) / 6));
          n.push(i === 100 ? '🌌 Сфера Бытия' : late[idx] + ' ' + (((i - 11) % 6) + 1));
        }
        return n;
      })(),
      /* Стадия, с которой включается «планетарная» форма: тело перестаёт
       * быть фигурой и превращается в почти сферическую массу, где голова
       * и конечности тонут в плоти. */
      cosmicStage: 40,
    ...
    soft: {
      ...
      /* --- v2.0 «God-Tier»: многослойный жир и вторичная динамика --- */
      // Три слоя жира. Глубокий почти не колышется и держит форму,
      // поверхностный — самый мягкий и даёт «дрожь» после движения.
      layers: {
        deep:        { stiffness: 1.00, damping: 1.00, amp: 0.30 },
        medium:      { stiffness: 0.55, damping: 0.70, amp: 0.44 },
        superficial: { stiffness: 0.30, damping: 0.52, amp: 0.26 },
      },
      layerTransfer: 0.62,        // сколько силы глубокий слой отдаёт верхнему
      // Остаточные колебания. Подобрано так, чтобы большой живот дрожал
      // ~1.5–2.5 с после удара: дольше выглядит как желе и раздражает.
      momentumTrail: 0.30,
      microJiggle: 0.055,         // амплитуда микро-дрожи в покое
      heartbeatAmp: 0.011,        // видимая пульсация сердца
      waveSpeed: 3.6,             // м/с — скорость бегущей волны по телу
      // (после подгонки коллайдеров зоны стали компактнее, и при 7.5 м/с
      //  волна пробегала тело за один кадр — каскад переставал читаться)
      thermalRate: 0.85,          // скорость нагрева трущихся зон
      thermalCool: 0.28,          // скорость остывания
      sweatThreshold: 0.55,       // нагрев, с которого выступает пот
      volumePreserve: 0.42,       // сохранение объёма при сжатии
      adaptiveSettle: 0.06,       // «оседание» жира при долгом стоянии
    },
  ...
  const ZONES = [
    ...
    Z('upper_belly', 'Верхний живот', 'belly', [0, 1.28, 0.20], [0, 0.15, 1], 0.58, 0.95, SPEED.FAST, 0.85, 0.30, 22, true,
    ...
    Z('lower_belly', 'Нижний живот', 'belly', [0, 0.82, 0.24], [0, -0.35, 1], 0.62, 1.20, SPEED.FAST, 0.93, 0.22, 34, true,
      { folds: [12000, 45000], sound: 'ploh' }),
    Z('apron_fold', 'Apron fold (нижний фартук)', 'belly', [0, 0.60, 0.20], [0, -0.9, 0.6], 0.54, 1.05, SPEED.SLOW, 0.97, 0.16, 28, true,
      { folds: [30000, 90000], sound: 'flop', shelter: true }),
    Z('left_flank', 'Левый бок', 'belly', [-0.40, 1.05, 0.02], [-1, 0, 0.15], 0.42, 0.86, SPEED.FAST, 0.9, 0.26, 20, true),
    Z('right_flank', 'Правый бок', 'belly', [0.40, 1.05, 0.02], [1, 0, 0.15], 0.42, 0.86, SPEED.FAST, 0.9, 0.26, 20, true),
    Z('muffin_top', 'Muffin top', 'belly', [0, 0.96, -0.02], [0, 0.45, 0.1], 0.46, 0.58, SPEED.MEDFAST, 0.88, 0.28, 12, true,
      { folds: [9000] }),
    Z('side_belly_folds', 'Боковые складки живота', 'belly', [-0.48, 0.88, 0.10], [-1.0, -0.25, 0.35], 0.30, 0.82, SPEED.MEDSLOW, 0.92, 0.24, 11, true,
      { mirror: 'right', folds: [15000] }),
    Z('subrib_fold', 'Подрёберная складка', 'belly', [0, 1.34, 0.16], [0, -0.3, 1], 0.34, 0.62, SPEED.MEDSLOW, 0.86, 0.30, 9, true),
    Z('navel', 'Пупок (углубление)', 'belly', [0, 1.00, 0.36], [0, 0, 1], 0.13, -0.16, SPEED.MEDSLOW, 0.99, 0.35, 3, true,
      { inverted: true, sound: 'squish' }),

    /* ---------- БЛОК 2: ЯГОДИЦЫ И БЁДРА (10) ---------- */
    // Ягодицы вынесены дальше назад (z -0.34) и растут преимущественно
    // НАЗАД: раньше вектор роста был почти вертикальным вниз, из-за чего
    // попа «оплывала» вместо того, чтобы выпирать. gain поднят до 1.45.
    Z('lower_left_glute', 'Нижняя левая ягодица', 'glutes', [-0.25, 0.80, -0.34], [-0.30, -0.15, -1.5], 0.48, 1.70, SPEED.LIGHTNING, 0.94, 0.21, 30, true,
      { folds: [10000, 40000], sound: 'ploh' }),
    Z('lower_right_glute', 'Нижняя правая ягодица', 'glutes', [0.25, 0.80, -0.34], [0.30, -0.15, -1.5], 0.48, 1.70, SPEED.LIGHTNING, 0.94, 0.21, 30, true,
      { folds: [10000, 40000], sound: 'ploh' }),
    Z('upper_left_glute', 'Верхняя левая ягодица', 'glutes', [-0.23, 1.00, -0.34], [-0.22, 0.18, -1.4], 0.42, 1.25, SPEED.FAST, 0.9, 0.24, 18, true),
    Z('upper_right_glute', 'Верхняя правая ягодица', 'glutes', [0.23, 1.00, -0.34], [0.22, 0.18, -1.4], 0.42, 1.25, SPEED.FAST, 0.9, 0.24, 18, true),
    Z('back_shelf', '«Полка» над попой', 'glutes', [0, 1.14, -0.34], [0, 0.42, -1.2], 0.42, 0.62, SPEED.MEDFAST, 0.87, 0.27, 12, true,
      { platform: true }),
    ...
    Z('left_cheek', 'Левая щека', 'face', [-0.13, 1.97, 0.13], [-0.85, -0.25, 0.7], 0.15, 0.30, SPEED.MEDFAST, 0.8, 0.32, 3, true, { sound: 'squish' }),
    Z('right_cheek', 'Правая щека', 'face', [0.13, 1.97, 0.13], [0.85, -0.25, 0.7], 0.15, 0.30, SPEED.MEDFAST, 0.8, 0.32, 3, true, { sound: 'squish' }),
    Z('chin1', 'Первый подбородок', 'face', [0, 1.88, 0.15], [0, -0.55, 1], 0.14, 0.32, SPEED.MEDFAST, 0.9, 0.26, 4, true),
    Z('chin2', 'Двойной подбородок', 'face', [0, 1.80, 0.14], [0, -0.75, 0.9], 0.15, 0.40, SPEED.MEDSLOW, 0.93, 0.22, 6, true, { folds: [14000] }),
    Z('chin3', 'Тройной подбородок', 'face', [0, 1.72, 0.12], [0, -0.9, 0.8], 0.16, 0.46, SPEED.VERYSLOW, 0.95, 0.20, 8, true, { folds: [40000] }),
    Z('front_neck', 'Передняя шея', 'neck', [0, 1.70, 0.06], [0, -0.2, 1], 0.13, 0.28, SPEED.MEDSLOW, 0.9, 0.25, 7, true),
    Z('nape', 'Загривок', 'neck', [0, 1.78, -0.14], [0, 0.2, -1], 0.22, 0.34, SPEED.SLOW, 0.86, 0.28, 9, true),
    Z('side_neck_rolls', 'Боковые шейные валики', 'neck', [-0.19, 1.78, 0], [-1, 0, 0], 0.18, 0.24, SPEED.SLOW, 0.9, 0.26, 6, true, { mirror: 'right' }),
    Z('muzzle_lips', 'Губы / мордочка', 'face', [0, 1.98, 0.24], [0, -0.1, 1], 0.12, 0.10, SPEED.SLOW, 0.85, 0.30, 2, true),
    Z('brow_ridges', 'Надбровные валики', 'face', [0, 2.10, 0.16], [0, 0.5, 0.9], 0.12, 0.08, SPEED.VERYSLOW, 0.8, 0.32, 2, true),
    ...
    Z('left_elbow', 'Левый локоть', 'arms', [-0.52, 1.28, -0.02], [-1, 0, -0.3], 0.13, 0.14, SPEED.SLOW, 0.78, 0.32, 3, true),
    Z('right_elbow', 'Правый локоть', 'arms', [0.52, 1.28, -0.02], [1, 0, -0.3], 0.13, 0.14, SPEED.SLOW, 0.78, 0.32, 3, true),
    Z('left_paw', 'Левая лапа', 'arms', [-0.60, 0.98, 0.04], [-1, -0.4, 0.2], 0.14, 0.14, SPEED.SLOW, 0.8, 0.30, 3, true, { sound: 'squish' }),
    Z('right_paw', 'Правая лапа', 'arms', [0.60, 0.98, 0.04], [1, -0.4, 0.2], 0.14, 0.14, SPEED.SLOW, 0.8, 0.30, 3, true, { sound: 'squish' }),
    ...
    Z('left_knee', 'Левое колено', 'legs', [-0.24, 0.42, 0.06], [-0.6, 0, 0.8], 0.16, 0.18, SPEED.SLOW, 0.8, 0.30, 5, true),
    Z('right_knee', 'Правое колено', 'legs', [0.24, 0.42, 0.06], [0.6, 0, 0.8], 0.16, 0.18, SPEED.SLOW, 0.8, 0.30, 5, true),
    ...
    Z('left_ankle', 'Левая лодыжка', 'legs', [-0.24, 0.12, 0], [-0.8, 0, 0], 0.11, 0.12, SPEED.SLOW, 0.8, 0.30, 3, true),
    Z('right_ankle', 'Правая лодыжка', 'legs', [0.24, 0.12, 0], [0.8, 0, 0], 0.11, 0.12, SPEED.SLOW, 0.8, 0.30, 3, true),
    ...
    Z('spine_groove', 'Позвоночная борозда', 'back', [0, 1.35, -0.30], [0, 0, -1], 0.10, -0.14, SPEED.SLOW, 0.98, 0.34, 3, true, { inverted: true }),
    ...
    Z('left_foot', 'Левая стопа', 'misc', [-0.24, 0.05, 0.08], [0, -0.4, 0.8], 0.14, 0.10, SPEED.VERYSLOW, 0.85, 0.30, 3, true),
    Z('right_foot', 'Правая стопа', 'misc', [0.24, 0.05, 0.08], [0, -0.4, 0.8], 0.14, 0.10, SPEED.VERYSLOW, 0.85, 0.30, 3, true),
    Z('front_midline', 'Передняя центральная линия', 'misc', [0, 1.15, 0.34], [0, 0, 1], 0.12, -0.10, SPEED.VERYSLOW, 0.97, 0.33, 3, true, { inverted: true }),
  ];

  /* ============================================================
   * 3b. ФИЗИЧЕСКИЕ ПРОФИЛИ ЗОН (v2.0 «God-Tier»)
   * ------------------------------------------------------------
   * Базовая таблица (pos/dir/gain/soft) описывает ФОРМУ зоны.
   * Профиль ниже описывает её ПОВЕДЕНИЕ:
   *
   *   jiggle   — сила колыхания (0 нет .. 1 максимум)
   *   sag      — свисание под гравитацией (0 держит форму .. 1.5 висит)
   *   layers   — сколько слоёв жира моделируется (1 плотный .. 3 сочный)
   *   pend     — маятниковость: инерция вбок при ходьбе/повороте
   *   cell     — целлюлит (визуальная текстура, 0..1)
   *   fric     — трётся о соседей => нагрев, звук, пот
   *   micro    — множитель микро-дрожи в покое
   *   breath   — насколько зона участвует в дыхании
   *
   * Значения выставлены по «Общей таблице физики» из ТЗ:
   * живот/грудь/ягодицы — максимум, конечности — сдержанно,
   * загривок/плечи/полка — растут вверх и НЕ свисают.
   * ============================================================ */
  const P = (jiggle, sag, layers, pend, cell, fric, micro, breath) =>
    ({ jiggle, sag, layers, pend, cell, fric, micro: micro || 0, breath: breath || 0 });

  const ZONE_PHYSICS = {
    /* ЖИВОТ: главная зона — максимум колыхания, свисания и слоёв */
    upper_belly:        P(0.80, 0.50, 3, 0.45, 0.00, 0, 0.9, 1.00),
    mid_belly:          P(1.00, 1.00, 3, 0.85, 0.00, 0, 1.0, 0.75),
    lower_belly:        P(0.90, 1.30, 3, 0.95, 0.05, 0, 0.8, 0.35),
    apron_fold:         P(0.70, 1.50, 3, 1.00, 0.10, 1, 0.6, 0.15),
    left_flank:         P(0.90, 0.30, 2, 0.80, 0.15, 0, 0.7, 0.20),
    right_flank:        P(0.90, 0.30, 2, 0.80, 0.15, 0, 0.7, 0.20),
    muffin_top:         P(0.80, 0.60, 2, 0.70, 0.20, 0, 0.6, 0.20),
    side_belly_folds:   P(0.80, 0.40, 2, 0.65, 0.15, 1, 0.6, 0.15),
    subrib_fold:        P(0.40, 0.10, 1, 0.25, 0.00, 1, 0.5, 0.90),
    navel:              P(0.40, 0.50, 1, 0.15, 0.00, 0, 1.2, 0.85),

    /* ЯГОДИЦЫ И БЁДРА: батут-эффект, целлюлит, «полка» не свисает */
    lower_left_glute:   P(1.00, 0.80, 3, 0.90, 0.55, 0, 0.8, 0),
    lower_right_glute:  P(1.00, 0.80, 3, 0.90, 0.55, 0, 0.8, 0),
    upper_left_glute:   P(0.90, 0.40, 2, 0.70, 0.40, 0, 0.6, 0),
    upper_right_glute:  P(0.90, 0.40, 2, 0.70, 0.40, 0, 0.6, 0),
    back_shelf:         P(0.50, 0.00, 2, 0.35, 0.25, 0, 0.4, 0),
    outer_left_thigh:   P(0.80, 0.40, 2, 0.85, 0.50, 0, 0.6, 0),
    outer_right_thigh:  P(0.80, 0.40, 2, 0.85, 0.50, 0, 0.6, 0),
    upper_left_thigh:   P(0.80, 0.60, 3, 0.70, 0.45, 0, 0.6, 0),
    upper_right_thigh:  P(0.80, 0.60, 3, 0.70, 0.45, 0, 0.6, 0),
    undergluteal_folds: P(0.60, 0.90, 2, 0.50, 0.35, 1, 0.5, 0),

    /* ГРУДЬ: маятник, свисание, трение в складках */
    left_moob:          P(1.00, 1.00, 3, 0.95, 0.10, 0, 0.9, 0.55),
    right_moob:         P(1.00, 1.00, 3, 0.95, 0.10, 0, 0.9, 0.55),
    upper_chest:        P(0.40, 0.20, 2, 0.30, 0.00, 0, 0.7, 1.00),
    under_chest_folds:  P(0.50, 0.60, 2, 0.40, 0.05, 1, 0.5, 0.60),

    /* ЛИЦО, ШЕЯ: каскад подбородков с задержкой, загривок вверх */
    left_cheek:         P(0.60, 0.50, 2, 0.35, 0.00, 0, 1.1, 0),
    right_cheek:        P(0.60, 0.50, 2, 0.35, 0.00, 0, 1.1, 0),
    chin1:              P(0.50, 0.70, 2, 0.45, 0.00, 0, 0.9, 0.20),
    chin2:              P(0.70, 1.00, 2, 0.60, 0.00, 1, 0.9, 0.20),
    chin3:              P(0.70, 1.20, 2, 0.75, 0.00, 1, 0.8, 0.15),
    front_neck:         P(0.30, 0.40, 1, 0.25, 0.00, 0, 0.8, 0.45),
    nape:               P(0.50, 0.00, 2, 0.30, 0.00, 0, 0.4, 0),
    side_neck_rolls:    P(0.40, 0.20, 1, 0.30, 0.00, 1, 0.6, 0),
    muzzle_lips:        P(0.20, 0.10, 1, 0.10, 0.00, 0, 1.0, 0),
    brow_ridges:        P(0.10, 0.00, 1, 0.05, 0.00, 0, 0.7, 0),

    /* РУКИ: «крылья» летучей мыши сильно колышутся и хлопают по бокам */
    left_biceps:        P(0.90, 1.00, 3, 1.00, 0.20, 1, 0.6, 0),
    right_biceps:       P(0.90, 1.00, 3, 1.00, 0.20, 1, 0.6, 0),
    left_forearm:       P(0.50, 0.30, 2, 0.55, 0.10, 0, 0.4, 0),
    right_forearm:      P(0.50, 0.30, 2, 0.55, 0.10, 0, 0.4, 0),
    left_elbow:         P(0.30, 0.20, 1, 0.25, 0.00, 0, 0.3, 0),
    right_elbow:        P(0.30, 0.20, 1, 0.25, 0.00, 0, 0.3, 0),
    left_paw:           P(0.30, 0.20, 1, 0.30, 0.00, 0, 0.5, 0),
    right_paw:          P(0.30, 0.20, 1, 0.30, 0.00, 0, 0.5, 0),

    /* НОГИ: внутренние бёдра трутся, икры — столбы, стопы плющатся */
    inner_left_thigh:   P(0.70, 0.30, 2, 0.45, 0.40, 1, 0.5, 0),
    inner_right_thigh:  P(0.70, 0.30, 2, 0.45, 0.40, 1, 0.5, 0),
    left_knee:          P(0.30, 0.20, 1, 0.25, 0.15, 0, 0.3, 0),
    right_knee:         P(0.30, 0.20, 1, 0.25, 0.15, 0, 0.3, 0),
    left_calf:          P(0.50, 0.30, 2, 0.40, 0.20, 0, 0.4, 0),
    right_calf:         P(0.50, 0.30, 2, 0.40, 0.20, 0, 0.4, 0),
    left_ankle:         P(0.30, 0.50, 1, 0.20, 0.10, 0, 0.3, 0),
    right_ankle:        P(0.30, 0.50, 1, 0.20, 0.10, 0, 0.3, 0),
    left_foot:          P(0.30, 0.20, 1, 0.20, 0.00, 0, 0.3, 0),
    right_foot:         P(0.30, 0.20, 1, 0.20, 0.00, 0, 0.3, 0),

    /* СПИНА: валики-ступени, борозда-жёлоб, плечи держат форму */
    upper_back:         P(0.50, 0.10, 2, 0.35, 0.00, 0, 0.5, 0.40),
    scapular_folds:     P(0.60, 0.30, 2, 0.45, 0.10, 1, 0.5, 0.25),
    lumbar_cushion:     P(0.60, 0.50, 2, 0.55, 0.30, 0, 0.5, 0),
    spine_groove:       P(0.30, 0.00, 1, 0.20, 0.00, 0, 0.4, 0.20),
    left_shoulder:      P(0.50, 0.10, 2, 0.40, 0.00, 0, 0.4, 0.15),
    right_shoulder:     P(0.50, 0.10, 2, 0.40, 0.00, 0, 0.4, 0.15),

    /* ПРОЧЕЕ: хвост — тяжёлый маятник с максимальной инерцией */
    tail_base:          P(0.90, 1.00, 2, 1.00, 0.00, 0, 0.5, 0),
    front_midline:      P(0.30, 0.00, 1, 0.20, 0.00, 0, 0.5, 0.40),
  };

  // Профиль по умолчанию — если зону забыли описать, тело не сломается
  const DEFAULT_PHYSICS = P(0.5, 0.3, 2, 0.4, 0.1, 0, 0.5, 0.1);

  /* Overdrive — насколько зона продолжает расти ПОСЛЕ насыщения.
   * Без него тело замирало на 7-й стадии: дальше кормить было бессмысленно.
   * Живот и ягодицы разгоняются сильно, лицо и конечности — почти нет,
   * иначе у гиганта распухала морда и он терял узнаваемость. */
  const OVERDRIVE = {
    belly: 1.55, glutes: 1.15, chest: 0.95, thighs: 0.85,
    back: 0.55, legs: 0.45, arms: 0.40, neck: 0.30, misc: 0.35, face: 0.16,
  };

  // Вживляем профиль прямо в описание зоны: движку удобнее читать оттуда
  for (const z of ZONES) {
    const p = ZONE_PHYSICS[z.id] || DEFAULT_PHYSICS;
    z.overdrive = z.inverted ? 0.25 : (OVERDRIVE[z.group] !== undefined ? OVERDRIVE[z.group] : 0.5);
    // Главная зона живота растёт заметнее прочих
    if (z.main) z.overdrive = 2.1;
    z.jiggle = p.jiggle;
    z.sag = p.sag;
    z.layers = p.layers;
    z.pend = p.pend;
    z.cellulite = p.cell;
    z.friction = z.friction || !!p.fric;
    z.micro = p.micro;
    z.breath = p.breath;
  }
  ...
  // И у каждой должен быть физический профиль
  {
    const missing = ZONES.filter((z) => !ZONE_PHYSICS[z.id]).map((z) => z.id);
    if (missing.length) console.warn('[config] Зоны без физ-профиля:', missing.join(', '));
  }
  ...
  const FOOD = [
    ...
    /* Топовая позиция склада: миллион монет за 99 999 калорий.
     * Одна порция двигает друга сразу на несколько стадий — это
     * осознанный «чит за деньги» для тех, кто накопил на почте. */
    F('cosmic_feast', 'Космический Пир', '🌌', 99999, 1000000, 'colossal', 'склад',
      { legendary: true, noCraft: true }),
  ...
  const LOCATIONS = [
    ...
    // Дом 26×22 занимает z 63..85; enter — во дворе перед крыльцом
    { id: 'cottage',  name: 'Коттедж',             x: -62,  z: 74,   r: 26, color: 0xffb3c1, music: 'home', home: true, enter: [0, 20] },
    ...
    // --- СКЛАД ПРАДЕДА: стоит прямо НАПРОТИВ коттеджа, вход к входу ---
    // Коттедж: x -62, z 74, фасад смотрит в +Z. Склад — на той же оси,
    // в 48 м перед крыльцом (дом крупный: 26×22 + крыльцо + палисадник),
    // развёрнут входом к дому.
    { id: 'secretvault', name: '⭐ Склад Прадеда',            x: -62, z: 122, r: 14, color: 0x9b7bd4, music: 'ambient', enter: [0, -19] },
  ];
  ...
  const ACHIEVEMENTS = [
    ...
    { id: 'hugger', name: 'Крепкие объятия', desc: 'Обнять друга (Z)' },
    { id: 'slept_on_friend', name: 'Сладкий сон', desc: 'Проспать ночь на друге' },
    { id: 'inside_friend', name: 'Внутри друга', desc: 'Уснуть и утонуть в мягком коконе' },
    { id: 'picnic_on_friend', name: 'Стол накрыт', desc: 'Устроить пикник на «полке»' },
    { id: 'strongman', name: 'Силач', desc: 'Прокачать силу до ×2' },
