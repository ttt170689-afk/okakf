...
(function (global) {
  ...
  const CONFIG = {
    ...
    player: {
      ...
      climbSpeed: 1.5,
      jumpVelocity: 5.1,
      /* Батут на животе: стоя на друге, Space подбрасывает вверх.
       * Выключен — прыжок обычный. Механика «Попрыгать» из меню (E)
       * работает независимо от этого флага. */
      bellyTrampoline: false,
      gravity: -18.6,
      maxStamina: 100,
    ...
    // --- Рост ---
    /* Практическая «бесконечность» для запасов и стеков.
     *
     * Просили 1e38, но столько в JS не сосчитать: MAX_SAFE_INTEGER —
     * это 9.007e15, а за ним x+1 === x, и счётчик перестаёт меняться
     * (проверено). Берём 999 999 999 999 999 — почти предел точности
     * и при этом честно работающее число: вычитание единицы всегда даёт
     * результат на единицу меньше. Для игрока это неотличимо от «бесконечно». */
    bigStack: 999999999999999,

    growth: {
      caloriesToKg: 0.045,        // 1 калория контента = 45 г массы
      baseMassKg: 62,
      /* Потолок стадий. Раньше было 100, теперь 9 999 999. */
      maxStage: 9999999,
      /* Порог стадии в калориях.
       *
       * Раньше это был готовый массив из 101 числа. Для 10 млн стадий
       * так нельзя по двум причинам, обе проверены замером:
       *   1) массив на 9 999 999 чисел — это ~76 МБ только под пороги;
       *   2) прежний геометрический рост (×1.115) на стадии 223 выходит
       *      за Number.MAX_SAFE_INTEGER, и дальше целые числа считаются
       *      неточно — пороги начали бы совпадать между собой.
       *
       * Поэтому пороги считаются функцией, а рост после 10-й стадии
       * КВАДРАТИЧНЫЙ, а не геометрический: он никогда не упирается в
       * предел точности, но по-прежнему делает каждую стадию тяжелее
       * предыдущей. Массив ниже (stageThresholds) сохранён для старого
       * кода и тестов — в нём первые 101 значение. */
      stageCalories: (i) => {
        const head = [0, 250, 1200, 4500, 12000, 28000, 60000, 110000, 220000, 400000, 800000];
        if (i < head.length) return head[i];
        const k = i - 10;                 // 1, 2, 3, ...
        /* 800k + плавный квадратичный разгон.
         * Коэффициент 8.2 подобран так, чтобы порог САМОЙ последней
         * стадии (9 999 999) остался внутри Number.MAX_SAFE_INTEGER
         * с 11-кратным запасом: 8.2e14 против 9.0e15. */
        return Math.round(800000 + 92000 * k + 8.2 * k * k);
      },
      stageThresholds: (() => {
        const head = [0, 250, 1200, 4500, 12000, 28000, 60000, 110000, 220000, 400000, 800000];
        const t = head.slice();
        for (let i = 11; i <= 100; i++) {
          const k = i - 10;
          t.push(Math.round(800000 + 92000 * k + 8.2 * k * k));
        }
        return t;
      })(),
      ...
      /* Имя стадии. Как и с порогами, держать 10 млн строк в массиве
       * нельзя — считаем по запросу. Массив stageNames ниже остался
       * для старого кода: в нём первые 101 имя. */
      stageName: (i) => {
        const head = [
        ...
        if (i >= 9999999) return '🌌 Сфера Бытия';
        const idx = Math.floor((i - 11) / 6) % late.length;
        // На сверхдалёких стадиях к имени добавляется «виток» — иначе
        // после 105-й они начали бы повторяться дословно
        const loop = Math.floor((i - 11) / (late.length * 6));
        return late[idx] + ' ' + (((i - 11) % 6) + 1) + (loop > 0 ? ' · ур.' + loop : '');
      },
      stageNames: (() => {
        const n = [
          'Стройняшка', 'Мягонький', 'Пухляш', 'Полненький', 'Толстяк',
