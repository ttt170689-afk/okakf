...
console.log('=== 7. СТРОГАЯ ПРОВЕРКА «ПОД ЖИВОТОМ» ===');
{
  ...
  /* Если игрок ЗАЛЕЗ на тело, режим onbelly приоритетнее: над головой у
   * гиганта плоть есть почти всегда (грудь, подбородки нависают), но
   * стоя ногами на друге ты сверху, а не под ним. Проверяем именно это. */
  const onBody = !!p.standingZone;
  const isUnder = p.mode === 'underbelly';
  if (onBody) {
    t('стоя на теле режим — onbelly, а не under', p.mode === 'onbelly', p.mode);
  } else {
...
console.log('=== 8. ФОРМА: МНОГОЯРУСНОСТЬ ===');
{
  ...
  relief /= mx;
  /* Раньше здесь требовался высокий рельеф — тест писался, когда силуэт
   * набирался из колец. Но «ярусы» и оказались той самой стопкой блинов,
   * которую пришлось убрать: на артах живот — гладкая капля.
   *
   * Поэтому теперь проверяем ДРУГОЕ: тело не должно быть ни идеально
   * гладким шаром (нет характера), ни ступенчатым (стопка). Умеренный
   * рельеф от боковых складок — то, что нужно. */
  t('силуэт не безликий шар', relief > 0.10, 'рельеф=' + relief.toFixed(3));
  t('силуэт не ступенчатый', relief < 0.45, 'рельеф=' + relief.toFixed(3));
