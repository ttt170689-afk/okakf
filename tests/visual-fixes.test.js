...
console.log('=== 4. МОЖНО ПОДОЙТИ ВПЛОТНУЮ ===');
{
  ...
  // Двухслойная модель: жир мягко пускает, но внутренний слой держит.
  // Значит погружение должно быть заметным, но НЕ полным.
  t('погружение мягкое, но ограниченное', normal > 0.2 && normal <= 1.0,
    'sink=' + normal.toFixed(2));
  t('карабканье не мешает погружению', burrowing >= normal - 0.02,
